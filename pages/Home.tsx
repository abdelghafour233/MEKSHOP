    ...
    <div className="space-y-12 md:space-y-24 pb-20 bg-slate-50 dark:bg-black">
        ...
        <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                {[
                  ...
                  <div key={i} className="bg-white dark:bg-[#0a0a0a] p-8 md:p-10 rounded-[32px] border border-slate-100 dark:border-white/5 flex flex-col items-center text-center group hover:border-green-500/30 transition-shadow duration-300 shadow-xl">
                      <div className="bg-green-500/10 p-4 md:p-5 rounded-2xl md:rounded-3xl mb-6 md:mb-8 group-hover:scale-110 transition-transform">
                          {feature.icon}
                      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center md:items-end justify-between mb-10 md:mb-16 gap-4">
            ...
            <Link to="/products" className="text-green-600 dark:text-green-500 hover:text-green-400 font-black flex items-center gap-2 group text-base md:text-lg transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
                {homeCategories.map((item, idx) => (
                  <Link key={idx} to={`/products?category=${item.cat}`} className="relative h-[300px] md:h-[450px] rounded-[32px] overflow-hidden group border border-slate-200 dark:border-white/5 shadow-2xl">
                      <img src={item.img} alt={item.title} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-60 dark:opacity-40" />
