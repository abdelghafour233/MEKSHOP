...
import { ShoppingCart, Truck, ArrowRight, ShieldCheck, CheckCircle, AlertTriangle, Home } from 'lucide-react';
  ...
  useEffect(() => {
    if (product) {
      ...
      const schemaData = {
        ...
        "sku": product.id,
        "brand": { "@type": "Brand", "name": "berrima store" },
        "offers": {
          "@type": "Offer",
        ...
        <div className="max-w-md w-full text-center space-y-8 bg-white dark:bg-[#0a0a0a] p-12 rounded-[48px] shadow-3xl border border-slate-200 dark:border-white/5 animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-3">
            ...
            <p className="text-slate-500 dark:text-gray-500 font-bold leading-relaxed">
                يبدو أن هذا المنتج قد تم حذفه أو أنك تتصفح من جهاز جديد لم يتم مزامنة البيانات معه بعد.
            </p>
          </div>
          <div className="pt-4 flex flex-col gap-3">
              ...
              <Link to="/" className="text-slate-500 dark:text-gray-400 font-bold hover:text-green-500 transition-colors flex items-center justify-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-20 items-start">
          <div className="space-y-6 md:space-y-8">
            ...
            {gallery.length > 1 && (
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide md:grid md:grid-cols-5 md:gap-4 md:pb-0">
                    {gallery.map((img, idx) => (
                        <button key={idx} onClick={() => setActiveImage(img)} className={`flex-shrink-0 w-20 h-20 md:w-auto md:h-auto md:aspect-square rounded-2xl border-2 overflow-hidden transition-all ${activeImage === img ? 'border-green-600' : 'border-slate-200 dark:border-white/5 opacity-50'}`}>
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-4 pt-4">
                 {[{ icon: <Truck className="h-6 w-6" />, label: "توصيل مجاني" }, { icon: <CheckCircle className="h-6 w-6" />, label: "جودة مضمونة" }, { icon: <ShieldCheck className="h-6 w-6" />, label: "دفع آمن" }].map((item, i) => (
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="bg-white dark:bg-[#0a0a0a] p-8 rounded-[32px] border border-slate-200 dark:border-white/5 shadow-sm mb-10">
              <h2 className="font-black text-slate-900 dark:text-white mb-6 text-xl flex items-center gap-3"><div className="w-1.5 h-6 bg-green-600 rounded-full"></div> مميزات المنتج</h2>
